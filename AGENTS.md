...
### opencode.json 등록
사용자의 `opencode.json`에 다음 항목을 추가해야 합니다:
```json
{
  "mcp": {
    "md-publishing-mcp": {
      "type": "uvx",
      "command": ["--from", "git+https://github.com/DCode1119/MarkdownPublishingMCP", "md-publishing-mcp"],
      "enabled": true
    }
  }
}
...
3. 사용자의 `opencode.json`을 읽고 `md-publishing-mcp` 항목이 있는지 확인
